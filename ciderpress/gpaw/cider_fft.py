...
from gpaw.xc.gga import gga_vars, add_gradient_correction, GGA
...
from ciderpress.dft.cider_kernel import \
    call_xc_kernel_gga, call_xc_kernel_mgga
...
class FTGradient:
    ...
    def __init__(self, gd, v, scale=1.0, dtype=float):
        ...
        self.Ga_G = np.ascontiguousarray(
            self.pd.get_reciprocal_vectors(q=0)[:, v]
        )
        if scale != 1.0:
            self.Ga_G *= scale
...
class CiderGGAHybridKernel(CiderKernel):
    ...
    def calculate(self, e_g, n_sg, v_sg, sigma_xg, dedsigma_xg, feat_sg):
        ...
        e_g_tmp, v_sg_tmp, dedsigma_xg_tmp = \
            np.zeros_like(e_g), np.zeros_like(v_sg), np.zeros_like(dedsigma_xg)
...
class CiderGGAHybridKernelPBEPot(CiderGGAHybridKernel):
    ...
    def calculate(self, e_g, nt_sg, v_sg, sigma_xg, dedsigma_xg, feat):
        ...
        self.call_xc_kernel(
            e_g, nt_sg, sigma_xg, feat,
            np.zeros_like(v_sg), np.zeros_like(dedsigma_xg), vfeat,
            RHOCUT=self.rhocut,
        )
...
class CiderMGGAHybridKernel(CiderGGAHybridKernel):
    ...
    def calculate(self, e_g, n_sg, v_sg, sigma_xg, dedsigma_xg,
        ...
        e_g_tmp, v_sg_tmp, dedsigma_xg_tmp, dedtau_sg_tmp = \
            np.zeros_like(e_g), np.zeros_like(v_sg), \
            np.zeros_like(dedsigma_xg), np.zeros_like(dedtau_sg)
        if self.ckernel.type == 'GGA':
            self.ckernel.calculate(e_g_tmp, n_sg, v_sg_tmp, sigma_xg, dedsigma_xg_tmp)
        else:
            self.ckernel.calculate(e_g_tmp, n_sg, v_sg_tmp,
                                   sigma_xg, dedsigma_xg_tmp,
                                   tau_sg, dedtau_sg_tmp)
        e_g[:] += e_g_tmp
        v_sg[:] += v_sg_tmp
        dedsigma_xg[:] += dedsigma_xg_tmp
...
def construct_cubic_splines(self):
    ...
    self.dense_lambd = (self.bas_exp[-1]/self.bas_exp[0])**(1.0/(self.dense_Nalpha-1))
    self.dense_bas_exp = self.bas_exp[0]*self.dense_lambd**(np.arange(self.dense_Nalpha))
...
def get_cider_coefs(self, cider_exp):
    ...
    return cho_solve(self.alpha_cl,  cvec)[self.alphas].reshape(n,*shape) * norm[self.alphas], \
           cho_solve(self.alpha_cl, dcvec)[self.alphas].reshape(n,*shape) * norm[self.alphas]


def get_cider_exp_gga(self, rho, sigma,
                      a0=1.0, fac_mul=0.03125, amin=0.0625,
                      return_derivs=False):
    sr = np.sign(rho)
    rho = np.abs(rho)
    ...
    if return_derivs:
        ...
        dadrho = sr * (2*B/(3*rhod**(1./3)) - 2 * sr * (C*tau/(rhod*rhod+1e-10)))
...
def get_cider_exp_mgga(self, rho, sigma, tau,
                       a0=1.0, fac_mul=0.03125, amin=0.0625,
                       return_derivs=False):
    CFC = 0.3 * (3 * np.pi**2)**(2.0/3)
    fac = fac_mul * 1.2 * (6 * np.pi**2)**(2.0/3) / np.pi
    if self.nspin==1:
        ...
        B = np.pi * (a0 - fac)
    C = np.pi / 2**(2./3) * fac / CFC
    ascale = B * rho**(2./3) + C * tau / (rho + 1e-10)
    cond0 = ascale<amin
    ascale[cond0] = amin*np.exp(ascale[cond0]/amin-1)
    cond1 = np.logical_or(ascale > self.maxen, np.isnan(ascale))
    cond2 = ascale < self.minen
    ...
    if return_derivs:
        cond3 = np.logical_or(cond1, cond2)
        dadrho = 2*B/(3*rho**(1./3)+1e-10) - (C*tau/(rho*rho+1e-10))
...
class _CiderBase:
    ...
    def __init__(self, cider_kernel, nexp, consts,
                 Nalpha=None, lambd=2.0, encut=80,
                 world=None, **kwargs):
    ...
    def get_mlfunc_data(self):
        return yaml.dump(self.cider_kernel.mlfunc.to_dict(),
                         Dumper=yaml.CDumper)
    ...
    def _setup_cd_xd_ranks(self):
        ...
        self.world.gather(np.array([cd], dtype=np.int32),
            ROOT, rank_cd if self.world.rank == 0 else None)
        ...
        self.world.gather(np.array([xd], dtype=np.int32),
            ROOT, rank_xd if self.world.rank == 0 else None)
    ...
    def _setup_kgrid(self):
        ...
        if self.a_comm is not None:
            self.gdfft = GridDescriptor(self.shape, self.gd.cell_cv * scale_c1,
                                        True, comm=self.comms['d'])
            ...
            for v1 in range(3):
                for v2 in range(3):
                    stress_vv[v1,v2] = self.pwfft.integrate(
                        ...
                        dG2deps_vvQ[v1,v2], P_k, global_integral=False,
                        hermitian=True,
                    )
    ...
    def calculate_6d_integral_bwd(self, n_g, v_g, vexp, vfeat_g,
                                  p_iag, q_ag, dq_ag,
                                  augv_abi=None, compute_stress=False):
        nexp = self.nexp
        alphas = self.alphas
    ...
    def calc_cider(self, e_g, nt_sg, v_sg, sigma_xg, dedsigma_xg,
        ...
        for s in range(nspin):
            feat[s], dfeat[s], p_iag[s], q_ag[s], dq_ag[s] = \
                self.calculate_6d_integral_fwd(
                    cider_nt_sg[s], ascale[s]
                )
        ...
        for s in range(nspin):
            ...
            self.calculate_6d_integral_bwd(
                cider_nt_sg[s], v_sg[s], vexp[s], vfeat[s], p_iag[s],
                q_ag[s], dq_ag[s], compute_stress=compute_stress)
        self.timer.stop()
    ...
    @staticmethod
    def from_mlfunc(mlfunc, slx='GGA_X_PBE', slc='GGA_C_PBE',
        ...
        return cls(cider_kernel, nexp, const_list, Nalpha=Nalpha,
                   lambd=lambd, encut=encut, xmix=xmix)
...
class CiderGGA(_CiderBase, GGA):
    ...
    def __init__(self, cider_kernel, nexp, consts, **kwargs):
        ...
        self.type = 'GGA'
        self.name = 'CIDER_{}'.format(self.cider_kernel.xmix) # TODO more informative name
...
class CiderMGGA(_CiderBase, MGGA):
    ...
    def get_ascale_and_derivs(self, nt_sg, sigma_xg, tau_sg):
        ...
        for s in range(nspin):
            ngcond = nt_sg[s] < self.RHO_TOL
            for i in range(nexp):
                ascale[s,i], dadn[s,i], dadsigma[s,i], dadtau[s,i] = self.get_cider_exp(
                    nt_sg[s], sigma_xg[2*s], tau_sg[s],
                    a0=self.consts[i,1],
                    fac_mul=self.consts[i,2],
                    amin=self.consts[i,3],
                    return_derivs=True,
