...
from ciderpress.dft.sphere_util import gauss_fft, gauss_conv_coef_and_expnt
...
PFUNC2_NORMS = np.array(
    [5.77240697e+00, 3.07271389e+00, 3.14265261e+00, 2.13897490e+01,
     1.94289398e+01, 1.33574495e+02, 1.14971278e+02, 7.98033330e+02,
     6.65701240e+02, 4.65664626e+03, 3.80496685e+03],
    dtype=np.float64,
)
...
def get_phi_iabg(phi_iabk, ls, rgd):
    ...
    for i in range(n_pfuncs):
        for a in range(nalpha):
            for b in range(nbeta):
                phi_iabg[i,a,b,:] = rgd.transform_single_bwd(phi_iabk[i,a,b,:], ls[i])
    return phi_iabg
...
def get_p12_p21_matrix(delta_lpg, phi_jabg, rgd, llist_j, w_b):
    ...
    for j in range(nj):
        l = llist_j[j]
        p12_pbja[:,:,j,:] = np.einsum('pg,abg,g->pba', delta_lpg[l], phi_jabg[j], dv_g)
...
def get_delta_lpg(betas, rcut, rgd, thr=6, lmax=4):
    ...
    rd_g = rgd.r_g - rcut
    poly = fcut[0,:,None] + rd_g * fcut[1,:,None] + rd_g**2 * fcut[2,:,None] / 2 + rd_g**3 * fcut[3,:,None] / 6
...
def get_delta_lpk(delta_lpg, rgd):
    ...
    for l in range(delta_lpg.shape[0]):
        for p in range(delta_lpg.shape[1]):
            delta_lpk[l,p] = rgd.transform_single_fwd(np.ascontiguousarray(delta_lpg[l,p]), l)
    return delta_lpk
...
def get_fit(l=0, RMAX=1.0, rs=None):
    ...
    funcs = np.stack(
        [get_pfunc(2*i+l, rs, RMAX) for i in range(S0)]
    )
    ffuncs = np.stack(
        [get_ffunc(2*i+l, rs, RMAX) for i in range(S0)]
    )
...
def convolve_on_rgd(funcs, gmax, rs):
    ...
    for r_in in rs_in:
        ...
        fit_grid = einsum('iacg,jbcg->iajb', convs, convs * dv_out)

def fpgp_test_func():
    lambd = 1.8
    emax = 3000
    Nalpha = 25
    alphas = emax / lambd**np.arange(Nalpha)
