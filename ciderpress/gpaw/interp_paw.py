    ...
    def from_setup(cls, setup, build_kinetic=False):
        ...
        if setup.Z > 36 and hasattr(setup.rgd, 'a') and hasattr(setup.rgd, 'b'):
            ...
            rgd = AERadialGridDescriptor(
                setup.rgd.a, setup.rgd.b, setup.rgd.N
            )
            gcut = rgd.ceil(rcut)
            rgd = rgd.new(gcut)
        else:
        ...
        return cls(
            ...
            _interpc(core_dens['nc_corehole_g']) if xcc.nc_corehole_g is not None else None,
            _interpc(core_dens['dnc_g']),
            _interpc(core_dens['dnct_g']),
            _interpc(core_dens['dnc_corehole_g']) if xcc.nc_corehole_g is not None else None,
            xcc.B_pqL,
...
def calculate_cider_paw_correction(expansion, setup,
                                   D_sp, dEdD_sp=None,
    ...
    res = expansion(rgd, D_sLq, xcc.n_qg, xcc.d_qg,
                    nc0_sg, dc0_sg, ae=True)
    rest = expansion(rgd, D_sLq, xcc.nt_qg, xcc.dt_qg,
                     nct0_sg, dct0_sg, ae=False)
...
class CiderRadialExpansion:
    def __init__(self, rcalc, F_sbgr=None, y_sbLg=None, f_in_sph_harm=False):
        ...
        if F_sbgr is not None:
            ...
            if f_in_sph_harm:
                F_sbLg = F_sbgr
                self.F_sbg = np.einsum('sbLg,nL->sbng', F_sbLg, Y_nL).reshape(nspins,nb,-1)
            else:
                self.F_sbg = F_sbgr.transpose(0,1,3,2).reshape(nspins,nb,-1)
            self.y_sbg = np.einsum('sbLg,nL->sbng', y_sbLg, Y_nL).reshape(nspins,nb,-1)
        else:
            self.F_sbg = None
            self.y_sbg = None
        assert self.rcalc.mode in ['feature', 'energy', 'potential',
                                   'semilocal', 'debug', 'debug_nosph']
    ...
    def __call__(self, rgd, D_sLq, n_qg, dndr_qg, nc0_sg, dnc0dr_sg, ae=True):
        ...
        if self.rcalc.mode == 'feature':
            ...
            x_sbg = self.rcalc(
                rgd, n_sLg, Y_nL[:,:Lmax],
                dndr_sLg, rnablaY_nLv[:,:Lmax], ae=ae
            )
            nb = x_sbg.shape[1]
            wx_sbng = x_sbg.reshape(nspins, nb, Y_nL.shape[0], -1)

            return 4 * np.pi * np.einsum('sbng,nL->sbgL', wx_sbng, weight_n[:,None] * Y_nL)
            ...
            if self.F_sbg is None:
                ...
                feat_xg = self.rcalc(
                    rgd, n_sLg, Y_nL[:, :Lmax],
                    dndr_sLg, rnablaY_nLv[:, :Lmax],
                    self.F_sbg, self.y_sbg, ae=ae
                )
            ...
            if self.rcalc.mode == 'debug_nosph':
                ...
                return 4 * np.pi * np.einsum('...ng,nL->...gL', feat_xng, weight_n[:,None] * Y_nL)
            ...
            for n, Y_L in enumerate(Y_nL[:, :Lmax]):
                ...
                e_g, dedn_sg, b_vsg, a_sg, dedsigma_xg = \
                    self.rcalc(rgd, n_sLg, Y_L, dndr_sLg, rnablaY_Lv)
                dEdD_sqL += np.dot(rgd.dv_g * dedn_sg,
                                   n_qg.T)[:, :, np.newaxis] * (w * Y_L)
                dEdD_sqL += 2 * np.dot(rgd.dv_g * dedsigma_xg[::nspins] * a_sg,
                                       dndr_qg.T)[:, :, np.newaxis] * (w * Y_L)
                if nspins == 2:
                    dEdD_sqL += np.dot(rgd.dv_g * dedsigma_xg[1] * a_sg[::-1],
                                       dndr_qg.T)[:, :, np.newaxis] * (w * Y_L[:Lmax])
            ...
            """
            dedn_sg, b_vsg, a_sg, dedsigma_xg = \
                self.rcalc(rgd, n_sLg, Y_nL[:,:Lmax], dndr_sLg, rnablaY_nLv[:,:Lmax],
                           self.F_sbg, self.y_sbg, ae=ae)
            ...
            dEdD_sqL = np.einsum('snq,nL->sqL',
                dEdD_snq,
                weight_n[:,None] * Y_nL[:,:Lmax]
            )
            ...
            dEdD_sqL += 8 * np.pi * np.einsum('nLv,vsnq->sqL', weight_n[:,None,None] * rnablaY_nLv[:,:Lmax], B_vsnq)
            ...
            if mode == 'energy':
                e_g, dedn_sg, b_vsg, a_sg, dedsigma_xg, v_sbg = \
                    self.rcalc(rgd, n_sLg, Y_nL[:,:Lmax], dndr_sLg, rnablaY_nLv[:,:Lmax],
                               self.F_sbg, self.y_sbg, ae=ae)
                nb = v_sbg.shape[1]
            else:
            ...
            if mode == 'energy':
                ...
                v_sbgL = np.einsum('sbng,nL->sbgL', v_sbg.reshape(nspins,nb,nn,-1), weight_n[:,None] * Y_nL)
...
class CiderRadialFeatureCalculator:
    ...
    def __call__(self, rgd, n_sLg, Y_nL, dndr_sLg, rnablaY_nLv, ae=True):
        (e_g, n_sg, dedn_sg, sigma_xg, dedsigma_xg, a_sg,
         b_vsg) = vec_radial_gga_vars(rgd, n_sLg, Y_nL, dndr_sLg, rnablaY_nLv)
...
class CiderDebugFeatureCalculator:
    ...
    def __call__(self, rgd, n_sLg, Y_L, dndr_sLg, rnablaY_Lv, F_sbg=None, y_sbg=None, ae=True):
        (e_g, n_sg, dedn_sg, sigma_xg, dedsigma_xg, a_sg, b_vsg) = \
            vec_radial_gga_vars(rgd, n_sLg, Y_nL[:,:n_sLg.shape[1]],
                                dndr_sLg, rnablaY_nLv[:,:n_sLg.shape[1]])
        return self.xc.get_paw_atom_feat(n_sg, sigma_xg, y_sbg, F_sbg, ae,
                                         self.include_density)
...
class CiderDebugDensityCalculator:
    ...
    def __call__(self, rgd, n_sLg, Y_L, dndr_sLg, rnablaY_Lv, ae=True):
        (e_g, n_sg, dedn_sg, sigma_xg, dedsigma_xg, a_sg, b_vsg) = \
            vec_radial_gga_vars(rgd, n_sLg, Y_nL, dndr_sLg, rnablaY_nLv)
        return n_sg
...
class DiffGGA(GGA):
    ...
    def calculate_paw_correction(self, setup, D_sp, dEdD_sp=None,
        ...
        return calculate_cider_paw_correction(expansion,
                                              setup, D_sp, dEdD_sp,
                                              addcoredensity, a)
...
class DiffMGGA(MGGA):
    ...
    def calculate_paw_correction(self, setup, D_sp, dEdD_sp=None,
        ...
        # integrator object.
        E = calculate_cider_paw_correction(expansion,
                                           setup, D_sp, dEdD_sp,
                                           addcoredensity, a)
